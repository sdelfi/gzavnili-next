...
- `/api/bema/auth/{login,logout,me,refresh}` — login runs the lockout/attempt-logging logic (`src/lib/auth/login.ts`, mirroring the legacy thresholds), refresh rotates both tokens on every call (sliding session). On initial client load, `AuthProvider` retries `/me` after one refresh attempt; this lets a valid 7-day refresh cookie restore an expired 15-minute access cookie instead of spuriously showing the login page. If both cookies are invalid, the protected layout remembers the current internal bema URL in session storage and the login page returns there after successful authentication.
