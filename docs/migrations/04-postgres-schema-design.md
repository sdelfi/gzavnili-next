...
- Additionally maintain a `parcel_status_history` table (append-only: `parcel_id, status, changed_at, changed_by, reason`), written by the same trigger/function, so a status timeline exists regardless of which code path made the change and the admin UI can show it instead of only current state.
  - **Correction (2026-08-01):** this bullet originally justified the table as giving "a real audit trail — something the legacy system lacks outside the write-only `operations` table". That premise was false. Legacy has `ParcelHistory` — a per-field edit log with old/new values, the payment method/amount taken at that moment, and the acting admin's id — which §4 of [02-parcels-domain-analysis.md](02-parcels-domain-analysis.md) failed to inventory. It is richer than this table, the whole bema "Parcels Reports" screen is built on it, and it has since been ported as `parcel_history`. The two coexist: this one is trigger-written and can't be forgotten by a caller; that one carries the business detail a trigger can't know. See [../decisions/0018-parcel-edit-history.md](../decisions/0018-parcel-edit-history.md).
