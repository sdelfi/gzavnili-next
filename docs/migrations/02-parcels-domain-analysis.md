...
- **`ParcelHistory`** (**missing from this inventory until 2026-08-01** — the omission led [04-postgres-schema-design.md](04-postgres-schema-design.md) §1 to assert legacy had no audit trail, and a later port to be built on that false premise; see [../decisions/0018-parcel-edit-history.md](../decisions/0018-parcel-edit-history.md)): the real edit log — `historyID`, `parcelid`, `editdatetime`, `editstatus`, `oldvalue`, `newvalue`, `updater`, `valuename`, `payMethod`, `payAmount`, `updaterID`. One row per changed field, written from `MSSQLParcelDAO.create()`/`update()`/`doOperation()`, `parcels-update.cfm` and `money-collect.cfm`. The entire bema "Parcels Reports" screen reads from it, and it is the only record of what a payment was *at the moment it was taken* (the `parcels.payMethod1`/`payAmount1` columns are overwritten by later edits) and of which admin acted.
