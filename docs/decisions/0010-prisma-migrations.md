...
**Correction (found the hard way while adding the auth schema below):** trigger functions and `CHECK` constraints are genuinely invisible to Prisma's diffing — it has no schema-level concept of either, so they're never proposed for removal. **Indexes are not in that category.** Prisma's introspection *does* model indexes, so any index that exists in the database but isn't declared in `schema.prisma` (like the hand-added `pg_trgm` GIN indexes) shows up as drift on every future `prisma migrate diff`/`migrate dev` run and gets a `DROP INDEX` proposed for it — confirmed when generating the `add_bema_auth` migration below. **The actual policy going forward**: after generating a migration (via `--create-only`, or via `migrate diff` in non-interactive environments — see below), always read the generated SQL before applying it, and manually remove any `DROP INDEX` line that targets one of the hand-added indexes from the initial migration. This is reviewing/trimming an auto-generated file, not hand-authoring DDL — a materially different (and much smaller) act than what the AGENTS.md rule forbids. The `add_bema_auth` migration's SQL file has a comment documenting exactly this at the point it was trimmed.
...
## Generating migrations without a real TTY (agent/CI environments)

`prisma migrate dev` (interactive or `--create-only`) refuses to run at all in a non-interactive shell ("Prisma Migrate has detected that the environment is non-interactive") — this is what an AI agent's shell always is. The workaround used to generate `add_bema_auth`:

```
prisma migrate diff --from-migrations prisma/migrations --to-schema=prisma/schema.prisma --script > prisma/migrations/<timestamp>_<name>/migration.sql
```

This requires `datasource.shadowDatabaseUrl` to be set in `prisma.config.ts` (sourced from `SHADOW_DATABASE_URL` — see `.env.example`; create the throwaway database once with the `CREATE DATABASE` command documented there). `--from-migrations` replays the actual migration history (including the hand-written trigger/index SQL) into that shadow database before diffing, which is what surfaces the index-drift issue above — diffing `--from-config-datasource` (the live dev DB) instead produces the same result but for the wrong reason (it also includes any ad hoc state of the live DB, not just migration history) and should be avoided. After generating and trimming the file, apply it locally with `bun run db:migrate:deploy` (safe, non-interactive, and — as it happens — exactly the same command production uses), not `migrate dev`.
