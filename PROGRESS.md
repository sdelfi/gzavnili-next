...
- [x] **Parcel list/report follow-up** (2026-08-01): restored the exact filtered parcel
      count instead of the operationally useless `10,000+`; added shared `condensed` admin
...
- [x] **Admin sidebar brand link** (2026-08-01): `GZAVNILI` in the bema sidebar header is
      now a real internal link to `routes.home()` while retaining the existing brand styling
      — commit pending.
...
customer picker, trip date, service, AWB, content, store),`ParcelReceiverSection`    (receiver dropdown that refills the address block, GE-citizen name pairs, delivery
...
From-hour, and unstable pagination with no sort tiebreaker — full table in decision 0015. Schema additions (`parcels.pcode`, `parcels.b_paid_delivery`, `config.reg_awb`)
...
{login,logout,me,refresh}` route handlers. CSR-only client-side guard
      ...
      regardless of the client guard. Follow-up: initial session restoration now uses the
      valid refresh cookie when the short-lived access cookie expired; if re-authentication
      really is required, the guard remembers the complete bema path/query/hash and login
      returns there instead of always opening the users list — commit pending.
...
- [x] **Add Parcel** (2026-07-31, `docs/decisions/0017-bema-add-parcel.md`): the sidebar's
      ...
      behaviour on _both_ add screens but only enforced client-side/half-ported on the edit
      ...
      **Correction (same day, before anything shipped further):** the client asked "are you
      ...
      _formula_ but then used it for something legacy itself doesn't do (a proportional
      ...
      **Second correction, same day (client instruction: port legacy logic 1:1, bugs
      ...
      tracking-number _prefix_ specifically (the pricing half is now ported; the cosmetic
      ...
      **Resolved per client instruction ("everything as in legacy" for the split fields
      specifically):** the "Payment method 2"/two "Amount" fields stay in the UI exactly as
      legacy has them, confirmed inert (see docs/findings.md); the second method is optional
      unless a non-zero second amount was entered.
      Follow-up: fixed blank Amount 2 (`null`) incorrectly requiring Payment method 2 (legacy
...
- [x] Corrected "Add Parcel" (batch) visual parity — the previous pass's guess (no bordered
      ...
      same as legacy). Rebuilt the bottom panel to match legacy's actual grouping: "Add Parcel" + both Payment method pairs + Price Total + notifications + the batch's own "Save" button
      ...
      Weight/Value/Group row; Received/Received by and Notify live inside the receiver grid
      instead of floating between sections (commit pending).
...
- [ ] **Parcels: one thing worth confirming with the client** before it hardens into
      behaviour — the Debt filter is a _not-equal_ match on uninvoiced parcels (the value `0`
...
- [x] **Site Pages follow-up fixes** (same day): `/parcel-service.html` 404'd — `proxy.ts`'s
      middleware matcher excluded _any_ path with a dot (meant for static assets like
      `.css`/`.js`) which also excluded every `.html` CMS page; fixed to exclude by specific
      extension instead. `{CALCULATOR}` wasn't substituting — found a _second_,
      ...
      `{HELPTOSHOP}`/`{VOLUMECAL}`/`{NEXTSEND}`/`{NEXTDEL}` into _any_ page's content, not
...
input[type=text]` still sat in `style.css`): split into `src/components/ui/Input.css`
      input[type=text]`still sat in`style.css`): split into `src/components/ui/Input.css`    (the`input[type=...]`/`.datepicker`/`textarea`element styling — same already-decided
...
- [x] **`{QUOTEFORM}`/`{QUESTIONFORM}` CMS placeholders wired up** (client-reported:
      ...
      Affects `cargo.html` (both forms) and `parcel-service.html` (`{QUESTIONFORM}` only), en + ge. `{COURIERCALC_FORM}`/`{HELPTOSHOP}`/`{VOLUMECAL}` remain unported — each is a
...
- [ ] Remaining bema modules per the rollout plan: products, orders, statements, content,
      ...
      working link. Parcels' _list_ is done (above); its sibling screens are the open part —
      see the next item.
...
- [x] **Site Settings** (`bema/config/settings.cfm`/`vwSettings.cfm`) ported in full — the
      popup fields already existed (docs/decisions/0014); this pass added Header Site Message,
      Consignee, Airway Bill/Date, Regular/Express/Cargo Services dates+AWB, Lari Rate, and
      declared/non-declared parcel pricing (`Config` model gained `airwayBill`/`airwayDate`/
      `consignee`/`dtCargoShip`/`dtCargoEst`/`siteMessage`/`declaredPrice`/`nonDeclaredPrice`;
      `siteSettingsSchema`; `GET`/`PATCH /api/bema/config` expanded from popup-only to the full
      screen; `SiteSettingsForm` rebuilt with a `CollapsibleSection` per legacy `<h1>` group).
      `siteMessage2` confirmed write-only-and-dead (no input anywhere in the legacy view, no
      reader anywhere either) and not modeled. Fixed two stubs this surfaced once the real
      `airway.cfm`/`Config.cfc` source was available: `export-airway`'s hardcoded `CONSIGNEE`
      constant and wrong `regAwb`/`expAwb` fallback now read the real `config.consignee`/
      `airwayBill`/`airwayDate`, and `trip-info`'s Cargo dates (previously hardcoded `null` on the
      wrong assumption legacy had none) now read `config.dtCargoShip`/`dtCargoEst`. Both
      corrections and the `siteMessage2`/`siteMessage`/declared-pricing dead-vs-deferred
      distinctions are in `docs/findings.md`. Verified end-to-end against a real Postgres: GET/
      PATCH round-trip, trip-info's cargo dates, and export-airway's manifest header all confirmed
      correct with real config values.
- [x] **Payment Preferences** (`bema/config/payment.cfm`/`vwPaymentConfigForm.cfm`) ported —
      see docs/decisions/0020-payment-config.md. Most of the legacy view turned out to be
      HTML-commented-out (PayPal/PayFlowPro/Sage gateway blocks, Payment Methods, Credit Card
      Types, State Taxes, Fees, a duplicate Paypal-enable toggle); only what's actually live —
      a single-option Gateway select, Authorize.Net API Login/Transaction Key, and the four
      Paypal Express Checkout fields — is built (`Config` gained `gateway`/`gatewayLogin`/
      `gatewayTransKey`/`paypalUserId`/`paypalPassword`/`paypalTransactionKey`/`paypalEmail`;
      `paymentConfigSchema` ports `PaymentConfigFormValidation`'s real required/max-length
...
paymentmethods` and re-inserts only the two hardcoded lists, permanently losing any
      ...
      save doesn't disturb the Site Settings fields sharing the same `config` row.
- [x] **Messages / SMS list** (`bema/messages/messages.cfm`/`sms.cfm`) ported — see
      docs/decisions/0021-bema-messages.md. One new `Message` model backs both screens
      (`isSms` splits internal messages from outbound SMS, matching legacy's single shared
      `messages` table); reuses the pre-existing `MessageType` model for message categories.
      `GET /api/bema/messages` (search+chain+userId filters) plus `PATCH`/`DELETE` for the
      Active/Inactive toggle and Delete action; `GET /api/bema/sms` (read-only, matching
      legacy's own edit-link-commented-out view). `MessagesListPage`/`SmsListPage` at
      `/bema/messages`/`/bema/sms`, wired to the pre-existing Sidebar placeholders, modeled on
      the Site Pages CMS list (`PageListPage`) for the Table/Pagination pattern. Compose/
      reply/view screens and the separate `sms_queue` bulk-SMS-sending mechanism are out of
      scope (see the decision doc). Found and reproduced a display bug (the "Message" column
      shows the message-type label, not the body text) and two dead-code findings (four unused
